...
function validateOTPGanesh(){
	...
	if(cotp == otp){
		document.loginForm.action="regGanesh";
	...
	//event.preventDefault();
 	if(stage == 1){	
		...
		if(document.getElementById("adj_uscno").value.length != 0){
			if(!checkValidation("area_code","Please choose Area/Village",'error')){return false;}
		}else if(document.getElementById("noscnos").value.length !=0){
			if(!checkValidation("area_code","Please choose Area/Village",'error')){return false;}
		}else{
			if(!checkValidation("area","Please choose Area/Village",'error')){return false;}
		}		
...
function getPaymentsGanesh(){
	var conload=parseInt(document.newreg.reqload.value);
	
	if(document.newreg.reqload.value.length>0)
		{
			var conload=parseInt(document.newreg.reqload.value);
			if( conload <= 250)
			{
				document.newreg.othamount.value="500";
			}
			else if(conload>250 && conload<=500)
			{
				document.newreg.othamount.value="1000";
			}
			else
			{
				var dev1=(conload-500)/500;
				dev1=Math.ceil(dev1);
				document.newreg.othamount.value=1500+dev1*750;
			}
			
			document.getElementById("amount").innerHTML = document.newreg.othamount.value;
		}
	
}

function finalSubmit(stage,event){
	//var cnature = document.getElementById("xcnature").value;	
	
	if(!checkValidation("applicant_name","Please Enter Door No., Building Name",'error')){return false;}	
	if(!checkValidation("address1","Please Enter Door No., Building Name",'error')){return false;}	
	if(!checkValidation("address2","Please Enter Street",'error')){return false;}	
	if(!checkValidation("address3","Please Enter Area",'error')){return false;}	
	if(!checkValidation("address4","Please Enter Town",'error')){return false;}	
	...
	if(!checkValidation("reqload","Please Enter Required Load",'error')){return false;}	

	
			
	document.newreg.action="saveCCGanesh";
	document.newreg.enctype="multipart/form-data";	
	document.newreg.method="POST";
	document.newreg.submit();
